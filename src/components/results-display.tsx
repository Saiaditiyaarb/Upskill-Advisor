              ...
              {sortedCourses.map((c: any, i: number) => (
                <div key={c.course_id ?? i} className="border rounded-lg p-5 flex flex-col gap-3">
                  ...
                  {(c.skills && Array.isArray(c.skills) && c.skills.length > 0) && (
                    <div className="flex flex-wrap gap-2">
                      {c.skills.slice(0, 6).map((s: string, idx: number) => (
                        <Badge key={idx} variant="secondary" className="capitalize">{s}</Badge>
                      ))}
                    </div>
                  )}
                  ...
                  {(c.metadata?.description || c.metadata?.rating || c.metadata?.price) && (
                    <div className="text-sm text-muted-foreground space-y-1">
                      {c.metadata?.description && <p className="line-clamp-3">{c.metadata.description}</p>}
                      <div className="flex items-center gap-3 flex-wrap">
                        {c.metadata?.rating && <span>Rating: {c.metadata.rating}</span>}
                        {c.metadata?.price && <span>Price: {c.metadata.price}</span>}
                        {typeof c.metadata?.enrollment_count === 'number' && <span>Enrollments: {c.metadata.enrollment_count}</span>}
                      </div>
                    </div>
                  )}
      ...
      <Card>
        ...
        <CardContent>
          <details>
            ...
            <pre className="mt-3 text-xs bg-muted p-4 rounded-lg overflow-auto">{JSON.stringify(data, null, 2)}</pre>
          </details>
        </CardContent>
      </Card>
    </div>
